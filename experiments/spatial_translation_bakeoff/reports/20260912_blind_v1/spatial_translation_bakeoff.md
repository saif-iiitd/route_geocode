# Blind spatial-translation bake-off: 20260912_blind_v1

## Outcome

Completed blind-input preparation, three-arm preflight/smoke execution, immutable full-run status ledgers, candidate freeze and post-freeze audit reveal. **0 translations were generated.** A status ledger with 815 entries is not a completed translation arm. Unavailable providers were not substituted. No translation was fabricated, rewritten, or promoted to production.

The same 20 IDs, selected by seeded SHA-256 rank without labels, were assigned to every smoke arm. See `runs/20260912_blind_v1/smoke_report.json`. Blocked smoke gates are not successful API integration tests. The configuration is frozen for reproducibility of this blocked attempt; service integration must still pass a new smoke run after setup.

## System comparison

| System | Scheduled | Attempted translations | Successful | Missing/blocked |
| --- | ---: | ---: | ---: | ---: |
| indictrans2 | 815 | 0 | 0 | 815 |
| google | 815 | 0 | 0 | 815 |
| openai | 815 | 0 | 0 | 815 |
...
- indictrans2: MISSING_DEPENDENCY_torch; MISSING_DEPENDENCY_transformers; MISSING_DEPENDENCY_IndicTransToolkit; MISSING_DEPENDENCY_sentencepiece; GATED_MODEL_HF_CREDENTIAL_NOT_CONFIGURED
...
Blind input SHA-256: `8d8f5fcab9e03c6a1dfc90d5a2bfc7a3f92959df7f202b8e9bd65a169f2d9337`. Source commit: `62160b09b5d7dafee5e8d72912c9bda3cdb9d904`; working tree was dirty from the prior authorized task. The run snapshots generator source, schema, resolved configuration, prompt and library/Python versions. `freeze.json` hashes every frozen run artifact, including candidate CSVs and OpenAI alignment JSONL. Per-row output_sha256 hashes the UTF-8 translation, not the containing CSV. Missing outputs have empty output hashes. Timestamps are UTC; no timestamps are invented for unattempted generation.
